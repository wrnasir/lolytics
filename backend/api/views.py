...
class PredictionView(APIView):
    def post(self, request):
        ...
        if not game_name or not tag_line:
            return Response({'error': 'gameName and tagLine are required.'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            # Call the prediction method and return the response
            predictions = self.run_player_linear_regression(game_name, tag_line)
            return Response(predictions, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    
    def run_player_linear_regression(self, gameName, tagLine):
        """
        ...
        for matchId in matchIds:
            ...
            for participant in match_data['info']['participants']:
                if participant['puuid'] == puuid:
                    ...
                    previousGames.append({"championName": participant['championName'], 
                                        "kda": str(participant['kills']) + "-" +  str(participant['deaths']) + "-" + str(participant['assists']),
                                        "CS": str(participant['totalMinionsKilled']),
                                        "damage": str(participant['totalDamageDealtToChampions'])
                                        })
